...
def main():
    ...
    outfilepath         = mridata.get_connectivity_path("mrsi",parc_scheme,scale,npert,filtoption=preproc_string.replace("filt",""))
    ...
    try:
        ...
        fig, axs = plt.subplots(3,2, figsize=(16, 12))  # Adjust size as necessary
        plot_outpath = outfilepath.replace(".npz","_simmatrix")
        os.makedirs(outDirfigure_path,exist_ok=True)
        simplt.plot_simmatrix(simmatrix_sp,ax=axs[0,0],titles=f"MeSiM",
                            scale_factor=0.4,
                            parcel_ids_positions=parcel_ids_positions,
                            colormap="bluewhitered",show_parcels="H") 
        simplt.plot_simmatrix(simmatrix_sp_corr,ax=axs[0,1],titles=f"MeSiM corrected p>{alpha}",
                            scale_factor=0.4,
                            parcel_ids_positions=parcel_ids_positions,
                            colormap="bluewhitered",show_parcels="H") 
        ...
        simplt.plot_simmatrix(simmatrix_adjusted,ax=axs[1,0],titles=f"MeSiM [removed empty nodes]",
                            scale_factor=0.4,
        ...
        simplt.plot_simmatrix(simmatrix_binarized_pn,ax=axs[1,1],titles=f"Binarized MeSiM [density={density}]",
                            parcel_ids_positions=parcel_ids_positions,colormap="blueblackred",
                            scale_factor=0.6)
        # MeSiM vs Distance
        ...
        # RC plot
        axs[2,1].fill_between(node_deg, lower_rand_rc, upper_rand_rc, color='gray', alpha=0.5, label=f"Null Distribution (α={RC_ALPHA})")
